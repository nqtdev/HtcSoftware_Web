...
const normalizeItem = it =>
  typeof it === 'string'
    ? { image: it, alt: '' }
    : { ...it, image: it.image || it.slideContent || '', alt: it.alt || '' };
...
const DepthCarousel = ({
  ...
  onChange,
  className = '',
  cardClassName = 'bg-white shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-gray-100',
    ...
    >
      <div className="absolute inset-0 [transform-style:preserve-3d]" ref={stageRef}>
        {data.map((item, i) => (
          <div
            key={i}
            className={`absolute left-1/2 top-1/2 cursor-pointer overflow-hidden [transform:translate(-50%,-50%)] [transform-origin:center] [will-change:transform,opacity,filter] ${cardClassName}`.trim()}
          ...
          >
            <img
              className={`block h-full w-full select-none ${imageFit} [pointer-events:none] [-webkit-user-drag:none]`}
              src={item.image}
              alt={item.alt || ''}
              draggable={false}
            />
